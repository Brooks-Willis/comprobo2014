#!/usr/bin/env python
# Software License Agreement (BSD License)

import rospy
from statistics import stdev
from std_msgs.msg import String
from geometry_msgs.msg import Twist, Vector3
from sensor_msgs.msg import LaserScan

mean_distance = -1.0
min_angle = 0

def getch():
    """ Return the next character typed on the keyboard """
    import sys, tty, termios
    fd = sys.stdin.fileno()
    old_settings = termios.tcgetattr(fd)
    try:
        tty.setraw(sys.stdin.fileno())
        ch = sys.stdin.read(1)
    finally:
        termios.tcsetattr(fd, termios.TCSADRAIN, old_settings)
    return ch

def increment_angle(valid_ranges,index):
    if index > len(valid_ranges):
        loop_index = index - len(valid_ranges)
    elif index < 0:
        loop_index = index + len(valid_ranges)
    else:
        loop_index = index

    return loop_index


def scan_received(msg, pub, followdist):
    """ Processes data from the laser scanner, msg is of type sensor_msgs/LaserScan """
    global mean_distance
    global min_angle
    valid_ranges = []

    for i in range(len(msg.ranges)): 
        if msg.ranges[i] > 0 and msg.ranges[i] < 8:
            valid_ranges.append(msg.ranges[i])

    min_dist = min(valid_ranges)
    min_angle = valid_ranges.index(min)

    if len(valid_ranges) > 4:
        min_dist_error = [increment_angle(valid_ranges, min_angle-2),
        increment_angle(valid_ranges, min_angle-1),
        increment_angle(valid_ranges, min_angle),
        increment_angle(valid_ranges, min_angle+1),
        increment_angle(valid_ranges, min_angle+2)]
        if stdev(min_dist_error) > 0.1: #Is it a wall (need to add if not a wall, then what?)
            mean_distance = sum(min_dist_error)/float(len(min_dist_error))
        else: 
            mean_distance = followdist
    else:
        mean_distance = followdist

def approach_wall(followdist):
    pub = rospy.Publisher('cmd_vel', Twist, queue_size=10)
    sub = rospy.Subscriber('scan', LaserScan, scan_received, pub, followdist)
    rospy.init_node('teleop', anonymous=True)
    r = rospy.Rate(10) # 10hz
    while not rospy.is_shutdown():
        print min_angle
        if mean_distance != followdist:
            velocity_msg = Twist(Vector3(0.2*(mean_distance - followdist), 0.0, 0.0), Vector3(0.0, 0.0, 0.0))
        pub.publish(velocity_msg)
        r.sleep()

        
if __name__ == '__main__':
    followdist = rawinput("Input distance from wall (m)")
    try:
        approach_wall(followdist)
    except rospy.ROSInterruptException: pass
